...
export function Controls() {
  ...
  const currentPreset =
    qualityPresets.find((p) => p.value === config.quality) || qualityPresets[2]

  const handleSliderChange = (value: number[]) => {
    const quality = value[0]
    // Snap to nearest preset
    const nearestPreset = qualityPresets.reduce((prev, curr) =>
      Math.abs(curr.value - quality) < Math.abs(prev.value - quality)
        ? curr
        : prev
    )
    setConfig((prev) => ({
      ...
      <div className="mt-4 space-y-4">
        ...
        <div className="rounded-lg bg-muted/50 p-3">
          <p className="text-xs text-muted-foreground">
            ...
            WebP - optimized for web with excellent compression
          </p>
        </div>
      </div>
    </div>
